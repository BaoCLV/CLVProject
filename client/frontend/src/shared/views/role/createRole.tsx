...
const CreateRole = ({ onClose }: CreateRoleProps) => {
  ...
  const { loading: permissionsLoading, error: permissionsError, permissions } = usePermissions();  // Fetch permissions
  ...
  if (permissionsError) {
    return <p>Error loading permissions: {permissionsError.message}</p>;
  }
